#include <stdio.h>  
 ...
 #define BLANK ' '  
 int top = -1;  
 void push(long int symbol);  //defining push func
 long int pop();  
 ...
 long int stack[MAX];  // assigning array stack
 char *reverses(char str[]);  
 char str_tmp[100]; //temp array 
 ...
 {  
   ...
   {  
     ...
     {  
       switch (symbol)  
       {  
       case '(':  // checking the operator precedence
         push(symbol);  
         break;  
         //PEMDAS = Parenthesis > expo > * / > + -
       case ')':  
         while ((next = pop()) != '(')  
         {  
           postfix[p++] = next;  
